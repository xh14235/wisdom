<template>
  <div class="echarts-kuang" @click="showEchartsPopup({data: gaugeData, type: 'gauge'})">
    <div :id="gaugeData.id" class="echarts-id"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    drawBar (gaugeData) {
      ...
      let myChart = this.$echarts.init(document.getElementById(gaugeData.id))
      var option = {
        ...
        series: [
          {
            ...
            max: gaugeData.max, // 最大的数据值,默认 100 。映射到 maxAngle。
            ...
            axisLine: {
              ...
              lineStyle: {
                // 仪表盘轴线样式。
                color: [[0, this.green], [1, '#27544B']], // 仪表盘的轴线可以被分成不同颜色的多段。默认取值：[[0.2, '#91c7ae'], [0.8, '#63869e'], [1, '#c23531']]
                opacity: 1, // 图形透明度。支持从 0 到 1 的数字，为 0 时不绘制该图形。
                width: axisLineWidth, // 轴线宽度,默认 30。
                shadowBlur: 0, // (发光效果)图形阴影的模糊大小。该属性配合 shadowColor,shadowOffsetX, shadowOffsetY 一起设置图形的阴影效果。
                shadowColor: this.white // 阴影颜色。支持的格式同color。
              }
            },
            ...
            splitLine: {
              ...
              length: splitLineLength, // 分隔线线长。支持相对半径的百分比,默认 30。
              lineStyle: {
                // 分隔线样式。
                color: this.green, // 线的颜色,默认 #eee。
                opacity: 1, // 图形透明度。支持从 0 到 1 的数字，为 0 时不绘制该图形。
                width: splitLineWidth, // 线度,默认 2。
            ...
            detail: {
              ...
              fontSize: fontSize, // 文字的字体大小,默认 15。
              fontWeight: 600,
              formatter: '{value}' + gaugeData.unit // 格式化函数或者字符串
            },
