  ...
  methods: {
    ...
    conusedline() {
      conusedline().then(res => {
        ...
        const hotWater = Object.values(res.data.HOT_WATER).slice(
          0,
          this.day.length
        );
        const electry = Object.values(res.data.ELECTRICITY).slice(
          0,
          this.day.length
        );
    ...
    conusedpie() {
      ...
      conusedpie({
        ...
        let coldSum = parseInt(
          cold.AGRICULTURE +
            cold.HOME_STAY +
            cold.HOUSE +
            cold.INDUSTRY +
            cold.PUBLIC_CONSTRUCTION
        );
        let hotSum = parseInt(
          hot.AGRICULTURE +
            hot.HOME_STAY +
            hot.HOUSE +
            hot.INDUSTRY +
            hot.PUBLIC_CONSTRUCTION
        );
        let hotWaterSum = parseInt(
          hotWater.AGRICULTURE +
            hotWater.HOME_STAY +
            hotWater.HOUSE +
            hotWater.INDUSTRY +
            hotWater.PUBLIC_CONSTRUCTION
        );
        let electrySum = parseInt(
          electry.AGRICULTURE +
            electry.HOME_STAY +
            electry.HOUSE +
            electry.INDUSTRY +
            electry.PUBLIC_CONSTRUCTION
        );
    ...
    conthird3() {
      conelectric().then(res => {
        this.datathird.echarts6 = {
          ...
          yName: "(kW)",
          data: [
            Object.values(res.data.FOREIGN_ELECTRICITY).slice(
              0,
              this.day.length
            ),
